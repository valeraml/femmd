...
class Plotter:
    def __init__(self, r, v, tris, xmax, ymax, type, I2, sigma=1, **kwargs):
        self.r = r
        self.v = v
        self.tris = tris
        self.dy = 0.1*ymax
    ...
    def update(self,steps,walls=None,title=None):
        if self.type == "pts":
            ...
            im=self.ax1.tripcolor(self.r[:,0],self.r[:,1],self.tris,facecolors=colors,mask=long_edges(x,y,t))
            #for x, y, c in zip(self.r[:,0],self.r[:,1], vv): self.ax1.add_artist(Circle(xy=(x,y),fill=False,lw=2,radius=self.radius, ec=(c*.9+.1,0,0)))

            #self.fig.colorbar(im,self.ax1,fraction=0.046, pad=0.04)
